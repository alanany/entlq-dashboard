...
const getAllCourses = async (req, res) => {
 ...
 const courses = await Course.find();  console.log(courses);
    // 'dashboard/index' هو المسار النسبي للملف داخل مجلد 'views'
    res.render('../views/dashboard/student/course-list', { title: 'كورسات الموقع', courses: courses});    
}
...
const getEnrolledSubscription = async (req, res) => {
 ...
 const subscription = await Subscription.find({studentId: req.user._id}).populate({
 ...
 const pendingRequests = await Subscription.countDocuments({ studentId:  req.user._id, status: 'pending' });
const totalRequests = await Subscription.countDocuments({ studentId:  req.user._id });
const acceptedRequests = await Subscription.countDocuments({ studentId:  req.user._id, status: 'confirmed' });


    // 'dashboard/index' هو المسار النسبي للملف داخل مجلد 'views'
    res.render('../views/dashboard/student/student_enrollment_requests.ejs', { title: 'طلباتى ', allRequests: subscription,stats: {
        totalRequests,
...
const getRequestDetails = async (req, res, next) => {
    ...
    try {
        ...
        console.log(request.selectedPriceOption,'request course selectedPriceOption');
        if (!request) {
        ...
        if (request.courseId && (request.status === 'paid' || request.status === 'confirmed')) {
...
const acceptedRequests = await Subscription.find({ studentId:  req.user._id, status: 'confirmed' }).populate({
                path: 'courseId', // قم بتعبئة الكورس أولاً
                // داخل الكورس المُعبَّأ، قم بتعبئة التصنيف
                populate: {
                    path: 'category', // اسم الحقل في موديل Course
...
module.exports = {
  getMySessionsPage,getStudentSettings,getStudentBillingPage,
  getstudentDashboard,
  getAllCourses,
  signup_get,
  login_get,
  ...
  getRequestDetails,getSessionWaitingRoom,getNearestSession
};
